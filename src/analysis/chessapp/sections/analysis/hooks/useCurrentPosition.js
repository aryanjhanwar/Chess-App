...
const useCurrentPosition = (engine) => {
  ...
  useEffect(() => {
    ...
    if (boardHistory.length <= gameHistory.length && gameHistory.slice(0, boardHistory.length).join() === boardHistory.map((m) => m.san).join()) {
    ...
    if (!position.eval?.opening) {
      for (const move of boardHistory.slice().reverse()) {
        ...
        const opening = openings.find((opening2) => opening2.fen === moveFen);
        if (opening) {
    ...
    if (!position.eval && engine?.getIsReady() && engine.name && !board.isCheckmate() && !board.isStalemate()) {
      const getFenEngineEval = async (fen, setPartialEval) => {
        if (!engine.getIsReady()) {
          throw new Error("Engine not ready");
        }
        const savedEval = savedEvals[fen];
        if (savedEval && savedEval.engine === engine.name && (savedEval.lines?.length ?? 0) >= multiPv && (savedEval.lines[0].depth ?? 0) >= depth) {
          const positionEval = {
            ...savedEval,
      ...
      const getPositionEval = async () => {
        const setPartialEval = (positionEval) => {
        ...
        const rawPositions = fens.map((_, idx) => {
