...
import { playGameStartSound, playMoveSound, playCaptureSound, playCheckSound, playCastleSound, playGameEndSound, playPromotionSound, setSoundPreferences } from "../utils/sounds";
...
const UI_STATES = {
  ...
  MULTIPLAYER_MODE_SELECT: 'multiplayer_mode_select',
...
const UI_STATE_CAPABILITIES = {
  ...
  [UI_STATES.MULTIPLAYER_MODE_SELECT]: {
    canMovePieces: false,
    canUseInGameActions: false,
    canStartGame: false,
    canOpenMultiplayer: false,
...
export default function PlayScreen({ currentUser, isMultiplayerGame, setIsMultiplayerGame, multiplayerSide, setMultiplayerSide, isMultiplayerStarted, setIsMultiplayerStarted, multiplayerNotice, setMultiplayerNotice, entryMode, setEntryMode, gameMode, setGameMode, p2p, appBackgroundStyle }) {
  ...
  const uiState = useMemo(() => {
    ...
    if (['/auth', '/login', '/signup'].includes(p)) return UI_STATES.AUTH;
    if (p === '/menu') return UI_STATES.MODE_SELECT;
    if (p === '/multiplayer') return UI_STATES.MULTIPLAYER_MODE_SELECT;
    if (p === '/multiplayer/lobby') return UI_STATES.MULTIPLAYER_LOBBY;
    if (p === '/test') return UI_STATES.TEST;
    if (p === '/admin') return 'admin';
    
    // Default to game mode for /play and anything else
  ...
  }, [location.pathname, isReviewMode, showGameOverUI, gameState]);
  ...
  return (
    ...
    >
      <Sidebar
        ...
        onPlaySelect={(mode) => { handlePlaySelectFromNav(mode); navigate('/play'); }}
        onOpenTest={() => navigate('/test')}
      />
      ...
      */}
      ...
      <MobileNavigation
        handlePlaySelectFromNav={(mode) => { handlePlaySelectFromNav(mode); navigate('/play'); }}
        gameMode={gameMode}
